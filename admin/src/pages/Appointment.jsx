...
//     setLoading(status);
//     try {
//       const data = await api(`${API_BASE}/admin/appointments/${appt._id}/status`, {
//         method: 'PATCH',
...
//     setLoading('delete');
//     try {
//       await api(`${API_BASE}/admin/appointments/${appt._id}`, { method: 'DELETE' });
//       onDelete(appt._id);
//       onClose();
//     } catch (e) {
//       alert(e.message);
...
//     try {
//       const data = await api(`${API_BASE}/admin/appointments/stats`);
//       setStats(data.data);
//     } catch (e) {
//       console.error(e);
//     }
...
//       const data = await api(`${API_BASE}/admin/appointments?${params}`);
...
/* ── API HELPER ─────────────────────────────────────────────────────────────
 * Always call as:  api('/admin/appointments/stats')
 * NEVER pass API_BASE inside the path — this function owns the base URL.
 * ────────────────────────────────────────────────────────────────────────── */
const _raw = import.meta.env.VITE_API_URL;
const API_BASE = (_raw && _raw !== 'undefined')
...
const api = async (path, options = {}) => {
  ...
  // Normalise path: strip any accidental full-URL prefix or duplicate /api
  let cleanPath = path
    .replace(/^https?:\/\/[^/]+/, '')  // remove any leading origin
    .replace(/^\/api\//, '/')           // /api/admin/... → /admin/...
    .replace(/^\/api$/, '/');           // edge-case bare /api
...
const DetailModal = ({ appt, onClose, onStatusUpdate, onDelete, isAdmin }) => {
  ...
  const handleStatus = async (status) => {
    setLoading(status);
    try {
      // ✅ Relative path only — api() builds the full URL
      const data = await api(`/admin/appointments/${appt._id}/status`, {
        method: 'PATCH',
        body: JSON.stringify({ status, adminNote: note }),
      });
      onStatusUpdate(data.data);
  ...
  const handleDelete = async () => {
    ...
    try {
      // ✅ Relative path only
      await api(`/admin/appointments/${appt._id}`, { method: 'DELETE' });
      onDelete(appt._id);
      onClose();
    } catch (e) {
      alert(e.message);
...
const Appointment = () => {
  ...
  /* fetch stats — ✅ relative path */
  const fetchStats = useCallback(async () => {
    try {
      const data = await api('/admin/appointments/stats');
      setStats(data.data);
    } catch (e) {
    ...
    <div style={{ padding: '24px', minHeight: '100%', background: C.bg }}>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 16, alignItems: 'center' }}>
        ...
        <select
          value={statusFilter}
          onChange={e => setStatusFilter(e.target.value)}
          style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, color: statusFilter ? C.text : C.muted, padding: '9px 12px', fontSize: 13, outline: 'none', fontFamily: 'DM Sans,sans-serif', cursor: 'pointer', minWidth: 130 }}
        >
          <option value="">All Statuses</option>
          <option value="pending">Pending</option>
          <option value="approved">Approved</option>
        ...
        <select
          value={purposeFilter}
          onChange={e => setPurposeFilter(e.target.value)}
          style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, color: purposeFilter ? C.text : C.muted, padding: '9px 12px', fontSize: 13, outline: 'none', fontFamily: 'DM Sans,sans-serif', cursor: 'pointer', minWidth: 160 }}
        >
          <option value="">All Purposes</option>
          {PURPOSE_OPTIONS.map(p => <option key={p} value={p}>{p}</option>)}
        </select>
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, overflow: 'hidden' }}>
        <div style={{
          display: 'grid', gridTemplateColumns: '1fr 120px 160px 110px 90px 52px',
          gap: 0, padding: '10px 16px', borderBottom: `1px solid ${C.border}`, background: C.surface,
        }}>
          {['Name / Contact', 'Purpose', 'Preferred Date', 'Submitted', 'Status', ''].map((h, i) => (
        ...
        {loading ? (
          <div style={{ padding: '48px', textAlign: 'center', color: C.muted, fontSize: 13 }}>Loading appointments…</div>
        ) : error ? (
          <div style={{ padding: '48px', textAlign: 'center', color: C.red, fontSize: 13 }}>{error}</div>
        ) : appointments.length === 0 ? (
          <div style={{ padding: '48px', textAlign: 'center', color: C.muted, fontSize: 13 }}>No appointments found.</div>
        ) : (
          appointments.map((appt, idx) => (
            <div
              key={appt._id}
            ...
            >
              <div style={{ minWidth: 0 }}>
                <p style={{ color: C.text, fontSize: 13, fontWeight: 700, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{appt.fullName}</p>
                <p style={{ color: C.muted, fontSize: 11, margin: '2px 0 0', fontFamily: 'DM Mono,monospace', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{appt.phone}</p>
              </div>
              <span style={{ color: C.textDim, fontSize: 11, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{appt.purpose}</span>
              <span style={{ color: C.textDim, fontSize: 11, fontFamily: 'DM Mono,monospace' }}>
                {appt.preferredDate || '—'}
                {appt.preferredTime && <span style={{ color: C.muted }}> · {appt.preferredTime}</span>}
              </span>
              <span style={{ color: C.muted, fontSize: 11, fontFamily: 'DM Mono,monospace' }}>{new Date(appt.createdAt).toLocaleDateString()}</span>
              <div><StatusBadge status={appt.status} /></div>
              <button
                onClick={() => setSelected(appt)}
                style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 16, flexWrap: 'wrap', gap: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            ...
            {Array.from({ length: pagination.pages }, (_, i) => i + 1)
              ...
              .map((p, i) =>
                p === '…'
                  ? <span key={`e-${i}`} style={{ color: C.muted, padding: '0 4px', display: 'flex', alignItems: 'center', fontSize: 13 }}>…</span>
                  : <PageBtn key={p} active={p === page} onClick={() => setPage(p)}>{p}</PageBtn>
              )}
            <PageBtn disabled={page === pagination.pages} onClick={() => setPage(p => p + 1)}><ChevronRight size={14} /></PageBtn>
          </div>
        </div>
      )}
