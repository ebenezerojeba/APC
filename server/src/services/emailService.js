...
class EmailService {
  constructor() {
    if (!process.env.RESEND_API_KEY) {
      logger.warn('⚠️  RESEND_API_KEY not set — emails will not send.');
      this.enabled = false;
      return;
    }
  ...
  async verify() {
    if (!this.enabled) return false;
    try {
      // Resend has no explicit verify() — a lightweight domains list call works
      await this.client.domains.list();
      logger.info('✅ Resend email service connected and ready.');
      return true;
    } catch (err) {
      logger.error('❌ Resend API key verification failed:');
      logger.error(`   Message: ${err.message}`);
      logger.error('   → Check your RESEND_API_KEY at https://resend.com/api-keys');
      return false;
    }
  }
  ...
  async send({ to, subject, html, text }) {
    ...
    try {
      ...
      if (error) {
        logger.error(`📧 Resend error → ${to}: ${error.message}`);
        return false;
      }

      logger.info(`📧 Email sent → ${to} [${data.id}]`);
      return true;
    } catch (err) {
      logger.error(`📧 Failed to send email → ${to}: ${err.message}`);
      return false;
    }
  }
